...
> [!NOTE]
> This section describes Chrome specifically. Brave is the primary browser for this project, and Edge implements the same API names over different models — see [the cross-browser addendum](#addendum-portability), which changes the architecture from "build on Chrome's AI" to "build on a capability-detected adapter".
...
## Addendum — portability, and what it costs {#addendum-portability}

*Added after the first read-through, when Brave was named as the primary browser. This supersedes the assumption in [What you can build on now](#builtin-ai) that Chrome's implementation is the foundation.*

`LanguageModel`, `Translator` and `LanguageDetector` are not Chrome APIs. They are W3C Web Machine Learning drafts, and Edge implements the same global names over entirely different models. That is better news than it first looks: **one code path covers Chrome, Edge and Brave.** Firefox is the genuine outlier.

| Browser | Inference surface | Free-form generation | What the user must do | Model |
|---|---|---|---|---|
| **Chrome 138+** | `LanguageModel` · `Summarizer` · `Translator` · `LanguageDetector` · `Writer` · `Rewriter` · `Proofreader` | Yes | Nothing | Gemini Nano |
| **Edge 138+ (Canary/Dev)** | Same global names; Translator + Language Detector stable in Edge 148 with 145+ languages | Yes | One flag | Phi-4-mini, or Aion-1.0-Instruct on weaker hardware |
| **Brave** | Chrome's, once enabled | Yes | Two flags plus a manual component download in `brave://components` | Gemini Nano |
| **Firefox** | `browser.trial.ml` — a different shape entirely | **No** — only text-classification, token-classification, question-answering, fill-mask, summarization | Grant a post-install permission | Transformers.js + ONNX, models cached in IndexedDB |
| **Any, portable** | Transformers.js v3 bundled into the extension, WebGPU with WASM fallback | Yes, at whatever size you ship | Nothing, but a download | Whatever you choose to ship |

Two consequences follow, and the second is the one that shapes the build.

**One adapter, five implementations.** Everything above the adapter is browser-agnostic; the adapter answers three questions — can you classify, can you translate, can you generate — and every caller degrades on the answer rather than branching on a browser name.

**The no-model tier is not a fallback, it is the default tier.** Chrome and Edge both document that the model is *not downloaded on a metered connection*, and Firefox cannot generate free-form text at all. So the honest reading is that the extension will frequently run with no generative model available, and the product has to be excellent in that state. That is achievable, because the deterministic intent router and every evidence provider are model-free by design — the model only ever improves disambiguation and phrasing.

```mermaid
flowchart TB
    A["Lookup request"] --> B["Deterministic router<br/>+ evidence providers<br/>(always available, no model)"]
    B --> C{"Adapter capability?"}
    C -->|"classify"| D["Sense disambiguation<br/>constrained to candidate list"]
    C -->|"translate"| E["Turkish gloss"]
    C -->|"generate"| F["One-line composition<br/>over retrieved evidence"]
    C -->|"none"| G["Ship the evidence as-is"]
    D & E & F & G --> H["Answer card"]
```

### Decisions now locked {#decisions-locked}

| Question | Decision | What it changes in the plan |
|---|---|---|
| Distribution | Personal, unpacked. Store publishing reconsidered later | `<all_urls>` review cost drops out of Phases 0–5. The lean split-script design stays anyway, because it is also a per-page performance decision |
| Browsers | Brave primary; Chrome and Edge first-class; Firefox where it does not cost desktop features | Adapter layer above. A second manifest target, not a second codebase |
| Turkish | No Turkish reasoning. Translate locally before and after where a gloss helps | Removes the cloud tier's main justification. `Translator` covers `tr` on all three Chromium browsers |
| Trigger | Open on selection **by default**, plus a modifier path | See [the trigger answer](#trigger-answer). The engineering moves from "when to open" to "when to stay quiet" |
| Codebase | From scratch | The existing tree becomes a source of ideas on equal footing with the competitors, not a base to extend |
| First feature | English word lookup | Confirms Phase 2's leading path |
| Writing APIs | In scope — Writer, Rewriter, Proofreader, Summarizer | Added as a later phase. They are free where the generative tier exists, and absent where it does not |
| Network right now | Metered | No model downloads. Phases 0–2 must be built and tested with zero model, which the design already requires |

### Answering the trigger question directly {#trigger-answer}

You asked how a hover trigger can express *which* phrase you mean, given that sometimes it is one word and sometimes several. It cannot, on its own — and that is why both triggers belong in the product rather than one replacing the other.

```oku-compare-grid
{"cards":[
 {"t":"Selection — exact, default on","b":"You drew the boundary, so there is no ambiguity about span. This stays the primary trigger and stays on by default, as you asked.\n\nThe engineering problem moves from *when do we open* to *when do we stay quiet* — the list below.","verdict":"good"},
 {"t":"Modifier + hover — fast, approximate","b":"No selection needed: hold the modifier, point at a word. The extension takes the token under the cursor and then **grows the span** to the longest known entry starting there — a collocation, a named entity, a multi-word term.\n\nThat is how Yomitan handles Japanese, and it transfers to English n-grams.","verdict":"good"},
 {"t":"Keyboard span refinement — the missing piece","b":"While the modifier is held, `→` extends the span one word to the right, `←` shrinks it, `Shift+→` extends left. The card re-queries live as the span changes.\n\nThis is the direct answer: hover gives you a good guess in zero gestures, and two keystrokes make it exact without ever touching the mouse.","verdict":"good"}
]}
```

Ten guards make selection-triggered opening quiet rather than intrusive. None of them is a setting you have to find; they are the default behaviour.

1. **Dwell, don't race.** Open ~250 ms after the selection settles, not on `mouseup`.
2. **Copy cancels.** `Cmd/Ctrl+C` within the dwell window suppresses the card entirely — copying is a different intention and it is the single most common false positive.
3. **Never mid-drag.** Nothing renders while the button is down.
4. **Length gate.** Beyond ~12 words, show the handle only. Long selections are quote-and-copy intent.
5. **Editable fields are off** unless explicitly enabled.
6. **Never cover the selection.** Place below, flip above near the viewport edge, never overlap the selected rect.
7. **Never steal focus or scroll.** The page keeps keyboard focus; `Esc` closes.
8. **Selections inside our own UI are ignored** — you are allowed to select the answer to copy it.
9. **Learn from dismissal.** Three dismissals with no interaction on one site in one session offers handle-only mode for that site.
10. **One-click site kill switch** in the card header, not buried in options.

### Context-aware disambiguation {#context-aware}

Your `planner` example is the sharpest test in the whole design, because it is the case where a generic answer is not merely thin but *wrong*: on a page about Apache Iceberg, `planner` is a query planner, not a person who plans events.

The page carries the answer, and reading it costs nothing:

```mermaid
flowchart LR
    P["Page, profiled once<br/>title · h1 · meta · breadcrumbs<br/>host · og:site_name<br/>top distinctive terms"] --> T["Topic profile"]
    L["Local context<br/>enclosing sentence<br/>nearest heading<br/>code-block ancestry"] --> T
    T --> B1["Bias the search<br/>topic terms appended to<br/>Wikipedia / Wikidata query"]
    T --> B2["Rank returned senses<br/>by overlap with topic + sentence"]
    T --> B3["Constrain the model<br/>pick 1 of N candidate senses,<br/>never invent one"]
    B1 & B2 & B3 --> R["Sense that fits this page"]
```

The three uses are deliberately ordered by how much they can go wrong. Search biasing and overlap ranking are deterministic, cheap, and work with no model. The model step only ever chooses among senses that a real source already returned, which keeps it inside the rule that governs the whole design: the model classifies, it never asserts.

## Risks, and what would change the plan {#risks}

- **The generative tier will be absent more often than the hardware numbers suggest.** Disk and VRAM are not the binding constraint — *connection type* is. Chrome and Edge both document that the model is not downloaded on a metered connection, and Firefox cannot generate free-form text at all. Phase 3 should open by logging `LanguageModel.availability()` across your actual browsers and networks rather than assuming the desktop case.
- **Brave's setup is manual and can regress.** Enabling it needs two flags plus a component download, and Brave has carried open issues about that component being missing on some platforms. The extension should detect the state and offer the exact steps rather than silently degrading — but it must never *require* them.
...
All open questions from the first draft are now answered — the answers, and what each one changes, are in [Decisions now locked](#decisions-locked). Nothing in this document is waiting on input.

Two questions the build will raise later, recorded here so they are not rediscovered:

| Question | When it becomes live | What decides it |
|---|---|---|
| Ship a portable model with Transformers.js, or rely only on the browser's built-in one? | After Phase 3, once there is a measurement of how often the built-in tier is actually available across your machines and browsers | Whether the built-in tier is available often enough to make a bundled download unnecessary. Bundling costs disk and a first-run download; not bundling costs capability on Firefox and on metered connections |
| Publish to the Chrome Web Store | Only if the tool becomes something you would hand to someone else | Publishing reintroduces the `<all_urls>` review cost the personal-use decision removed, and freezes the trigger behaviour against other people's expectations |
